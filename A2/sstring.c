...
void print(char *s, unsigned int *size) { // simple print function
    for (int i = 0; i < *size; i++ ) {
        printf("%c",s[i]);
    }
}
void display(char *s, unsigned int *size, unsigned int *capacity){
    printf("String: \"");
    print(s, size);
    printf("\"\nLength: %d",*size);
    printf("\nCapacity: %d\n",*capacity);
...
void read(char **s, unsigned int *size, unsigned int *capacity){
    *size = 0; //inputing completly new string so reset the size
    ...
    while ((x = getchar()) != EOF && (x == ' ' || x == '\n' || x == '\t' || x == '\r' || x == '\v' || x == '\f'));  //make sure we get ride of space char between var and the string
    int quote = 0;
    if (x == '"'){
    ...
    while (x != EOF){      //loop until eof
        if (quote){ 
            ...
            if (x == ' ' || x == '\n' || x == '\t' || x == '\r' || x == '\v' || x == '\f') { //if space char, stop loop
                break;
            }
        }
        if (*size == *capacity) {    // if arrat full, double capacity
...
void concadenate(char **t1, unsigned int *size1, unsigned int *cap1, char *t2, unsigned int size2, char *t3, unsigned int size3){
    *size1 = 0; // resetting var
    while (*cap1 < size2 + size3) { //making sure our array size of t1 is big enough for the process
...
int main(int argc, char const *argv[]){
    ...
    while (scanf(" %c", &x) != EOF) { //the space before %c in scanf indicate skiping whitespace before it, this function will loop until EOF
        if (x == 'q'){ // if the char is q, end the loop
            ...
            else{printf("Invalid SString target %c\n", x); break;}// if not a-d, tell the use its invalid
        }
        else if (x == 'p'){ // if p is receive, trigger the print step
            scanf(" %c",&x);//like in read, check what variable we will print
            if ( x >= 'a' && x <= 'd' ) print(var[x-'a'],&sizes[x-'a']);// like in read, we check if variable is valid and simply print its contents
            else {printf("Invalid SString target %c\n", x); break;}//if not a-d signal the user that its invalid
            printf("\n");
        }
        else if (x == 'd'){
            scanf(" %c",&x); //the space before %c in scanf indicate skiping whitespace before it, check what variable it will use
            if ( x >= 'a' && x <= 'd' ) display(var[x-'a'],&sizes[x-'a'], &caps[x-'a']); 
            else {printf("Invalid SString target %c\n", x); break;}// if not a-d, tell the use its invalid
        }
